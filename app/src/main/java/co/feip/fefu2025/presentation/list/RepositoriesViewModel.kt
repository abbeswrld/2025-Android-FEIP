        ...
        viewModelScope.launch {
            try {
                if (repo.isStarred) {
                    ...
                    updateRepositoryState(repo.id, false, repo.stars.toInt() - 1)
                } else {
                    val updatedRepo = starProjectUseCase(repo.id)
                    updateRepositoryState(repo.id, true, updatedRepo.stars.toInt())
                }
            } catch (e: Exception) {
